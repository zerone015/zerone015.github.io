...
또한 결국 쉘을 획득하기 위해서는 `system("/bin/sh")`를 실행해야 하며, 이를 위해 `main` 스택 프레임의 `ret`을 덮어써야 한다. 이 과정에서 전역 변수 `oob`와 스택 사이의 거리를 알아야 하므로 스택 주소 유출이 필요하다.

앞서 libc base를 유출했으므로, libc 내의 `__environ`을 통해 스택 주소(`envp`)를 얻을 수 있다. 그런데 이 프로그램은 `oob`를 기준으로 한 상대 거리로 임의 접근을 하기 때문에, `__environ`까지의 오프셋을 계산하려면 `oob` 자체의 절대 주소를 먼저 알아야 한다. `oob`는 `.data` 섹션에 위치한 전역 변수이기 때문에, 결국 바이너리 base 주소 유출이 선행되어야 한다.
...
`oob`에서 `ret`까지의 거리를 `envp - oob - 288`로 구할 수 있다.

이제 libc base와 바이너리 base를 모두 구하고 `oob`에서 `ret`까지의 거리 계산도 마쳤으므로, libc의 `pop rdi; ret` 가젯을 이용해 `system("/bin/sh")`를 호출하도록 ROP 체인을 구성하면 된다.
