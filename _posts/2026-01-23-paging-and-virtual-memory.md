...
페이징에서는 외부 단편화가 발생하지 않는다. 메모리가 동일한 크기의 페이지로 분할되어 있고 비연속적으로 할당될 수 있기 때문이다.

다만, 프로그램에 할당된 메모리가 항상 페이지 크기의 배수인 것은 아니므로 마지막 페이지의 일부가 낭비될 수 있다. 이를 내부 단편화라고 하며, 평균적으로 페이지 크기의 절반 정도가 낭비된다고 본다.

외부 단편화는 메모리 할당 패턴에 따라 예측이 어렵지만, 내부 단편화는 페이지 크기에 따라 그 범위를 예측할 수 있어 관리가 용이하다는 장점이 있다.

## MMU (Memory Management Unit)
---
모든 프로세스는 독립적인 가상 주소 공간을 갖는다. 프로세스가 포인터로 접근하는 주소는 실제 물리 주소가 아니라 가상 주소이며, 프로세스는 이 공간을 선형적인 형태로 인식하여 사용한다.

그러나 실제로 데이터를 읽고 쓰기 위해서는 물리 메모리의 실제 위치를 알아야 한다. 특히 페이징 기법에서는 물리 메모리에 비연속적으로 흩어진 페이지들이 프로세스의 연속적인 가상 주소 공간에 매핑되므로, 가상 주소를 실제 물리 주소로 변환하는 메커니즘이 필요하다. 이러한 주소 변환을 수행하는 전용 하드웨어를 MMU라고 한다.

## Address Translation Mechanism
---
가상 주소를 물리 주소로 변환하기 위해서는 두 주소의 매핑 정보가 필요하다. 이 정보를 담아두는 테이블을 페이지 테이블이라고 한다. 

페이지 테이블 엔트리(PTE)는 대상 물리 페이지의 기준 주소(Base Address)와 함께 유효성(Present), 권한(R/W/X), 접근 레벨(U/S), 수정 여부(Dirty) 등 메모리 보호와 관리를 위한 다양한 속성 비트를 포함한다.
...
이와 같은 계층적 구조를 통해 실제로 사용되지 않는 가상 주소 영역에 대해서는 페이지 테이블을 생성하지 않음으로써 메모리를 절약할 수 있다. 또한 각 페이지 테이블의 크기가 4KB로 페이지 크기와 동일하므로, 페이지 테이블 자체도 물리 메모리에 비연속적으로 할당할 수 있다.
...
1. **Access Page Directory**: 커널이 CR3 레지스터에 저장해둔 페이지 디렉토리의 기준 주소를 통해 디렉토리에 접근한다.
2. **Directory Indexing**: 상위 10비트로 페이지 디렉토리에서 대응되는 엔트리를 찾아, 페이지 테이블의 기준 주소를 얻는다.
3. **Page Table Indexing**: 중간 10비트로 페이지 테이블에서 엔트리를 찾아, 실제 물리 페이지의 기준 주소를 얻는다.
4. **Calculate Final Address**: 하위 12비트(Offset)를 물리 페이지 기준 주소에 더해 실제 데이터에 접근한다.
...
* **Fast Page Walk:** 만약 PDE, PTE가 CPU 캐시에 이미 올라와 있다면, 메인 메모리까지 가지 않고도 빠른 주소 변환이 가능하다.

* **TLB Entry Structure**
    * **VPN (Virtual Page Number):** 가상 주소의 페이지 번호. 검색의 key 역할을 한다.
    * **PPN (Physical Page Number):** 매핑된 실제 물리 페이지의 기준 주소.
    * **Attribute Bits:** 해당 페이지의 권한(R/W/X), 접근 레벨(U/S), 유효성(Present), Dirty 비트 등 보호와 상태 정보를 포함한다.
...
* **Entry Structure Change:** 4GB 이상의 물리 주소를 가리키기 위해 엔트리 크기를 8바이트로 늘렸다. 이를 통해 엔트리 내부에 기준 주소를 담는 필드를 확장하여 더 높은 주소 대역을 지정할 수 있게 했다.
* **3-Level Paging:** 엔트리가 커지면서 페이지 테이블 하나에 512개의 엔트리만 담기게 되었다. 이로 인해 인덱스가 9비트로 제한되었고, 전체 32비트 주소 처리를 위해 최상위에 2비트 계층이 추가된 3단계 페이징(2-9-9-12) 구조를 사용한다.
...
* **Huge Page (4MB)**: 페이지 크기가 4MB로 커지면서 페이지 테이블이 불필요해진다. 페이지 디렉토리 엔트리가 페이지 테이블을 가리키는 대신 4MB 물리 페이지를 직접 가리킨다.
* **PSE with PAE (2MB)**: PAE 환경에서는 페이지 디렉토리 엔트리가 8바이트로 확장되고 한 디렉토리당 512개 엔트리를 포함하므로, 대형 페이지 크기가 2MB로 조정된다.
* **Trade-off**
    * **Pros**
        * 페이지 워크 단계가 줄어들어 주소 변환이 빨라진다.
        * 단일 TLB 엔트리가 커버하는 메모리 범위가 크게 확대되어 TLB 히트율이 상승한다.
        * 더 큰 단위로 메모리를 관리하므로 커널이 추적해야 할 페이지 메타데이터 수가 대폭 감소한다.
        * 동일한 메모리 영역 접근 시, 4KB 페이지는 최대 1024번의 페이지 폴트가 발생할 수 있으나 4MB 페이지는 단 한 번만 발생한다.
    * **Cons** 
        * 내부 단편화가 커져 메모리 낭비 문제가 심화된다.
        * HDD 환경에서 4KB 페이지는 파일시스템 블록 크기와 일치하여 디스크상 연속 배치가 가능하지만, 4MB 페이지는 블록 크기보다 훨씬 커서 단편화 발생 시 탐색/회전 지연이 급증한다.
        * 이러한 이유로 스왑이 발생하지 않는 커널 공간이나, 충분한 메모리를 확보하여 스왑을 최소화하는 데이터베이스 등의 애플리케이션에서 주로 활용된다.
...
## Copy-on-Write (COW)
---
fork() 시스템 콜로 자식 프로세스를 생성한 직후 exec()을 호출하는 패턴이 매우 흔하다. 이 경우 exec()이 주소 공간을 완전히 새로운 프로그램으로 교체하므로, fork() 시점에 부모의 메모리를 자식에게 모두 복사하는 것은 불필요한 오버헤드가 된다.

COW는 쓰기가 실제로 발생하기 전까지는 페이지를 복사하지 않는 최적화 기법이다.

### Mechanism
1. **Initial Setup**: fork() 시 물리 페이지를 복사하지 않고 부모와 자식이 동일한 물리 페이지를 공유하도록 매핑한다.
2. **Permission Downgrade**: 쓰기 가능 페이지를 읽기 전용으로 설정한다. 커널은 PTE의 자유 비트를 활용하거나 별도 메타데이터로 해당 페이지가 COW 용도임을 표시한다.
3. **Reference Counting**: 각 물리 페이지마다 참조 카운트를 관리하여 몇 개의 프로세스가 공유하고 있는지 추적한다.

### Copy Triggering
공유 중인 페이지에 쓰기를 시도하면 MMU가 읽기 전용 속성을 확인하고 페이지 폴트를 발생시킨다. 페이지 폴트 핸들러는 해당 페이지가 COW 용도임을 확인한 뒤 참조 카운트에 따라 처리한다.

* **Reference Count = 1**: 복사 없이 페이지 권한을 쓰기 가능으로 변경하고 COW 표시를 제거한다.
* **Reference Count ≥ 2**: 새로운 물리 페이지를 할당하여 내용을 복사한 뒤 매핑을 갱신한다. 원본 페이지의 참조 카운트는 1 감소한다.
...
### Page Replacement
커널은 항상 일정 수 이상의 여유 페이지를 유지하려고 한다. 여유 페이지가 임계값 아래로 떨어지면, 커널은 기존 물리 페이지를 디스크로 내보내 공간을 확보한다. 이를 스왑 아웃(Swap-out)이라 한다.

Dirty 비트를 확인하여 스왑 아웃 대상 페이지의 내용이 수정되었다면 스왑 영역에 기록하고, 그렇지 않으면 단순히 버린다. PTE의 Present 비트는 비활성화되고, 기준 주소 필드에는 스왑 영역의 블록 번호가 저장된다. 해당 물리 페이지는 재사용 가능 상태가 된다.
...
### Reverse Mapping
가상 주소를 통해서는 언제든지 PTE에 접근할 수 있지만, 커널은 역으로 물리 주소를 기반으로 PTE에 접근해야 할 때가 있다. 예를 들어 공유 메모리 환경에서는 하나의 물리 페이지가 여러 프로세스의 PTE에 매핑되어 있을 수 있다. 이때 해당 페이지를 스왑 아웃하려면 매핑된 모든 PTE를 찾아 상태를 갱신해야 한다. 

이를 위해 커널은 각 물리 페이지가 어떤 PTE들에 매핑되어 있는지 추적하는 역방향 매핑 자료구조를 유지한다.
...
**Mechanism**
1. 하드웨어는 페이지 접근 시마다 PTE의 액세스 비트를 1로 설정한다
2. 커널은 교체가 필요할 때 물리 페이지 메타데이터 리스트를 순회한다
3. 각 메타데이터의 역방향 매핑 정보로 해당 PTE에 접근하여 액세스 비트를 확인한다
   - **Bit = 1**: 최근 참조되었으므로 0으로 재설정하고 다음 페이지로 넘어간다
   - **Bit = 0**: 이전 순회 이후 재참조되지 않았으므로 해당 페이지를 교체한다
...
#### Heuristic Approach
실제 시스템은 프로세스별 작업집합을 개별 관리하지 않고, Active/Inactive 리스트를 통해 시스템 전체의 할당된 페이지를 통합으로 관리한다. 시간 대신 거리를 측정한다.
