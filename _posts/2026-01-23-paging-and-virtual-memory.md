...
그러나 실제로 데이터를 읽고 쓰기 위해서는 물리 메모리의 실제 위치를 알아야 한다. 페이징 기법에서는 물리 메모리에 비연속적으로 흩어진 페이지들이 프로세스의 연속적인 가상 주소 공간에 매핑되므로, 가상 주소를 실제 물리 주소로 변환하는 메커니즘이 필요하다. 이러한 주소 변환을 수행하는 전용 하드웨어를 MMU라고 한다.
